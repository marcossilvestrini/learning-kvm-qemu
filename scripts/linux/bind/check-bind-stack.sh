...
#Variables
IP_MASTER="192.168.0.130"
IP_SLAVE="192.168.0.140"
IP_FORWARDING="192.168.0.141"
IP_CACHING="192.168.0.131"

# File for outputs testing
FILE_TEST=test/dnsbind/check-bind-stack.txt
LINE="------------------------------------------------------"
...
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_MASTER -l vagrant sudo cat /var/named/skynet.zone |
    grep -ws "IN      NS      ol9-bind-master.skynet.com.br." -A 14 >>$FILE_TEST
echo $LINE >>$FILE_TEST

## Validate file zone
echo -e "Validade file zone...\n" >>$FILE_TEST
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_MASTER -l vagrant sudo named-checkzone skynet.com.br /var/named/skynet.zone >>$FILE_TEST
echo $LINE >>$FILE_TEST

## Check type of records
echo -e "Check type records...\n" >>$FILE_TEST
dig -4 @$IP_MASTER skynet.com.br ANY | grep -ws "QUESTION SECTION" -A 1 >>$FILE_TEST
echo $LINE >>$FILE_TEST
dig -4 @$IP_MASTER skynet.com.br ANY | grep -ws "ANSWER SECTION" -A 20 >>$FILE_TEST
echo $LINE >>$FILE_TEST
dig -4 @$IP_MASTER skynet.com.br ANY | grep -ws "ADDITIONAL SECTION" -A 4 >>$FILE_TEST
echo $LINE >>$FILE_TEST

## Check DNSSEC
echo -e "Check DNSSEC for zone...\n" >>$FILE_TEST
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_MASTER -l vagrant \ 
    sudo dig skynet.com.br DNSKEY +multiline | grep "3600 IN" -A 3 >>$FILE_TEST
echo $LINE >>$FILE_TEST
...
## Check named.conf
echo -e "Check file /etc/named.conf...\n" >>$FILE_TEST
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_SLAVE -l vagrant sudo cat /etc/bind/named.conf.local | grep -ws "skynet.com.br" -A 14 >>$FILE_TEST
echo $LINE >>$FILE_TEST

## Check file zone
echo -e "Check file zone...\n" >>$FILE_TEST
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_SLAVE -l vagrant sudo ls -lt /var/cache/bind/skynet.zone >>$FILE_TEST
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_SLAVE -l vagrant \
    sudo named-compilezone -f raw -F text -o /tmp/skynet.txt skynet.com.br /var/cache/bind/skynet.zone
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_SLAVE -l vagrant sudo cat /tmp/skynet.txt >>$FILE_TEST
echo $LINE >>$FILE_TEST

## Check transference zones
echo -e "Check transference zones...\n" >>$FILE_TEST
#host mail.skynet.com.br $IP_MASTER >/dev/null 2>&1
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_SLAVE -l vagrant sudo rndc retransfer skynet.com.br
...
## Check named.conf
echo -e "Check file /etc/named.conf...\n" >>$FILE_TEST
sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$IP_FORWARDING -l vagrant sudo cat /etc/bind/named.conf.local | grep -ws "skynet.com.br" -A 14 >>$FILE_TEST
echo $LINE >>$FILE_TEST
